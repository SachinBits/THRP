#!/usr/bin/env python3
"""Run Decision Tree baselines for Fighter, Cargo, Helicopter, and Bomber datasets."""
...
def run_superclass(dataset_root: Path, out_dir: Path, superclass: str, test_size: float) -> Path:
    ...
    out_path = out_dir / "decision_tree" / f"metrics_{superclass}.json"
    save_json(out_path, payload)
    save_model(model, out_dir / "decision_tree" / f"decision_tree_{superclass}.joblib")
    return out_path


def main() -> None:
    parser = argparse.ArgumentParser(description="Run Decision Tree baselines for all aircraft superclasses")
    ...
    for superclass in args.superclasses:
        ...
        print(f"Saved Decision Tree results for {superclass}")
