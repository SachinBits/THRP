#!/usr/bin/env python3
"""Run KNN baselines for Fighter, Cargo, Helicopter, and Bomber datasets."""
...
def run_superclass(dataset_root: Path, out_dir: Path, superclass: str, test_size: float) -> Path:
    ...
    out_path = out_dir / "knn" / f"metrics_{superclass}.json"
    save_json(out_path, payload)
    save_model(model, out_dir / "knn" / f"knn_{superclass}.joblib")
    return out_path


def main() -> None:
    parser = argparse.ArgumentParser(description="Run KNN baselines for all aircraft superclasses")
    ...
    for superclass in args.superclasses:
        ...
        print(f"Saved KNN results for {superclass}")
