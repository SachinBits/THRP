#!/usr/bin/env python3
"""Run SVM baselines for Fighter, Cargo, Helicopter, and Bomber datasets."""
...
def run_superclass(dataset_root: Path, out_dir: Path, superclass: str, test_size: float) -> Path:
    ...
    model = SVC(kernel="rbf", probability=True, random_state=0, max_iter=1000)
    ...
    out_path = out_dir / "svm" / f"metrics_{superclass}.json"
    save_json(out_path, payload)
    save_model(model, out_dir / "svm" / f"svm_{superclass}.joblib")
    return out_path


def main() -> None:
    parser = argparse.ArgumentParser(description="Run SVM baselines for all aircraft superclasses")
    ...
    for superclass in args.superclasses:
        ...
        print(f"Saved SVM results for {superclass}")
